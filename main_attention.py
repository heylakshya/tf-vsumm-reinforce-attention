...
parser = argparse.ArgumentParser("Tensorflow code for unsupervised video summarization with REINFORCE using attention mechanism")
# Dataset options
...
parser.add_argument('--decay-rate', type=float, default=0.1, help="learning rate decay (default: 0.1)")
...
parser.add_argument('--save-dir', type=str, default='log_attention', help="path to save output (default: 'log_attention/')")
...
def main():
	...
	print("\n", model.summary(), "\n")
	print("Model size: {:.5f}M".format(np.sum([np.prod(v.shape) for v in model.trainable_weights])/1000000.0))

	if args.stepsize > 0:
		lr = tf.keras.optimizers.schedules.ExponentialDecay(args.lr, decay_rate=args.decay_rate, decay_steps=args.stepsize)
	else:
		lr = args.lr
	...
	for epoch in range(start_epoch, args.max_epoch):
		...
		for idx in idxs:
			...
			seq = tf.expand_dims(tf.convert_to_tensor(seq), axis=0) # input shape (1, seq_len, dim)
			...
			with tf.GradientTape() as tape:
				...
				cost = args.beta * (tf.math.reduce_mean(probs) - 0.5)**2 # minimize summary length penalty term [Eq.11]
	...
	model_save_path = osp.join(args.save_dir, 'model_epoch' + str(args.max_epoch))
...
def evaluate(model, dataset, test_keys):
	...
	for key_idx, key in enumerate(test_keys):
		...
		seq = tf.expand_dims(tf.convert_to_tensor(seq), axis=0)
		# seq_len = seq.shape[1]
		# if use_gpu: seq = seq.cuda()
		probs = model(seq, training=False)
